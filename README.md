...
DataSoap started as a concept to build a more advanced data scrubbing system.  Datasoap allows users to be able to upload their own CSV files and have those files cleaned (scrubbed) against the national do not call list (DNC) resulting in a clean list downloadable at the users convenience.  
...
<br><br>4. Quick Search/quick clean.  We wanted to implement a quick search feature where users could enter a phone number and search the database quickly without having to upload a file.  Coding the search query wasn't problematic but displaying the results proved to be tougher then we initially thought.  Given the constraints of flask, if we tried to have a search route we would have to leave the page just to display a quick message to the user.  Or we could reload the page with the message which we also wanted to avoid. Neither solution was palatable, so instead we looked outside of flask and used javascript and JSON.
<br>![alt tag](https://github.com/DigitalCrafts-HTX-11-2016-Cohort/DataSoap/blob/master/static/assets/git_screens/Screen%20Shot%202017-01-15%20at%2010.02.43%20PM.png)
...
<br><br>4. Pandas library in Python - Our initial upload process involved using the Pandas library built in function to read data.  It was working incredibly quickly but we quickly encountered an issue with our site hanging and not being able to load.  We worked on other solutions to try to resolve this issue and eventually realized the Pandas function/dependency was the cause of the hang.  The debuging process took us over 24 hours and it was a very important lesson to keep track of what libraries you are importing and making sure they aren't bogging down your system.<br>
![alt tag](https://github.com/DigitalCrafts-HTX-11-2016-Cohort/DataSoap/blob/master/static/assets/git_screens/pandas.png)
<br><br>5. Error handling.  In general, like any project you spend a fair bit of time with general error handling.  Because of all the different routes and databases, we knew we could run into issues if we didn't stay organized.  To this end, we organized the data in classes (users and filename) and were cognizant of how we wanted to route between our static pages and dynamic content.  We started the project by building a roadmap and although we made adjustments along the way, modeling our design this way made implementing those changes more agile.
<br>
